    ...
    <div class="max-w-5xl mx-auto px-4 py-6">
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            <div class="bg-white border border-lumot/20 rounded-2xl px-5 py-4 shadow-sm">
                ...
                <p class="text-[10px] text-gray mt-0.5">₱ {{ number_format($grouped['paid']->sum('assessment_amount'), 2) }}</p>
            </div>
            <div class="bg-white border border-lumot/20 rounded-2xl px-5 py-4 shadow-sm">
                ...
                <p class="text-[10px] text-gray mt-0.5">₱ {{ number_format($grouped['partial']->sum('assessment_amount'), 2) }}</p>
            </div>
            <div class="bg-white border border-lumot/20 rounded-2xl px-5 py-4 shadow-sm">
                ...
                <p class="text-[10px] text-gray mt-0.5">₱ {{ number_format($grouped['pending']->sum('assessment_amount'), 2) }} due</p>
            </div>
        </div>
        ...
        @if($grouped['paid']->isEmpty() && $grouped['partial']->isEmpty() && $grouped['pending']->isEmpty())
            <div class="bg-white border border-lumot/20 rounded-2xl px-6 py-16 shadow-sm text-center">
                <div class="w-12 h-12 bg-lumot/20 rounded-2xl flex items-center justify-center mx-auto mb-3">
                    <svg class="w-6 h-6 text-gray" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                    </svg>
                </div>
                <p class="text-sm font-extrabold text-green mb-1">No payment records yet</p>
                <p class="text-xs text-gray">Payments will appear here once your application has been assessed by the office.</p>
            ...
            @if($grouped['partial']->isNotEmpty())
                <div class="mb-8">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h2 class="text-xs font-extrabold text-green uppercase tracking-widest">Payment Received — For Final Approval</h2>
                    </div>
                    <div class="space-y-4">
