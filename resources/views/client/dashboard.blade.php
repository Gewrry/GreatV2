...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-4 pb-28 sm:pb-0">
                ...
                <div class="bg-orange-50 border border-orange-200 rounded-2xl p-5 shadow-sm">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-orange-700 mt-0.5">Some of your business permits have outstanding
                                balances. Settle them on time to keep your permit in good standing.</p>

                            <div class="mt-4 space-y-2">
                                @foreach ($pendingInstallmentApps as $app)
                                    <div
                                        class="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-orange-100 hover:border-orange-300 transition-colors">
                                        <div>
                                            <p class="text-[10px] font-bold text-orange-801 uppercase tracking-wider">
        ...
        {{-- Welcome Card --}}
        <div class="fade-up glass rounded-3xl shadow-xl shadow-teal-900/8 p-6 card-press my-5">
        ...
        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-lumot/10 flex items-center justify-between">
                ...
                <a href="{{ route('client.applications.index') }}"
                    class="text-[10px] font-bold text-logo-teal hover:underline underline-offset-4">View All →</a>
            </div>
                ...
                @forelse($applications as $app)
                    <div class="px-6 py-4 hover:bg-bluebody/30 transition-colors flex items-center justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <h3 class="text-sm font-black text-green truncate">
                                {{ $app->business->business_name ?? 'Untitled Business' }}</h3>
                            <div class="flex flex-wrap items-center gap-2 mt-1">
                                ...
                                <span
                                    class="text-[10px] font-black uppercase {{ $app->workflow_status === 'approved' ? 'text-logo-green' : ($app->workflow_status === 'rejected' ? 'text-red-500' : 'text-logo-teal') }}">
                                    {{ $app->workflow_status }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <form action="{{ route('client.applications.destroy', $app->id) }}" method="POST"
                ...
                @empty
                    <div class="px-6 py-10 text-center">
                        ...
                        <a href="{{ route('client.apply') }}"
                            class="text-xs text-logo-teal font-bold mt-2 inline-block">Start your first application →</a>
                    </div>
                @endforelse
            </div>
        </div>
