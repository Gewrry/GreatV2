        ...
        <div class="mb-6">
            ...
            <div class="flex items-start justify-between">
                ...
                @php
                    ...
                    $statusColor =
                        $statusColors[$application->workflow_status] ?? 'bg-gray-100 text-gray border-gray-200';
                @endphp
                ...
                @foreach ($trackerSteps as $s)
                    <div class="flex items-center">
                        ...
                        @if ($s['route'])
                            ...
                            @else
                                <div
                                    class="flex items-center gap-1.5 px-2 {{ $s['status'] === 'upcoming' ? 'cursor-not-allowed' : '' }}">
                        @endif

                        <div
                            class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-extrabold shrink-0
                        {{ $s['status'] === 'active'
                            ? 'bg-logo-teal text-white shadow-sm shadow-logo-teal/30'
                            : ($s['status'] === 'done'
                                ? 'bg-logo-green text-white ring-2 ring-logo-green/20'
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-5">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span
                        class="ml-auto text-[10px] font-bold text-red-500 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full">
                        ...
                        <div class="rounded-xl border {{ $doc ? $doc->status_color : 'border-lumot/30 bg-lumot/5' }} p-4"
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    @elseif($doc)
                                        <div
                                            class="w-8 h-8 rounded-lg bg-yellow-100 flex items-center justify-center shrink-0">
                                    ...
                                    @else
                                        <div
                                            class="w-8 h-8 rounded-lg bg-lumot/20 flex items-center justify-center shrink-0">
                                            <svg class="w-4 h-4 text-gray/40" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    @if ($doc)
                                        <span
                                            class="text-[10px] font-bold px-2 py-0.5 rounded-full border {{ $doc->status_color }} capitalize">
                                            {{ $doc->status }}
                                        </span>
                                    ...
                                    @if ($doc && in_array($application->workflow_status, ['draft', 'returned']) && !$doc->isVerified())
                                        <form
                                            action="{{ route('client.documents.destroy', [$application->id, $doc->id]) }}"
                                            method="POST" onsubmit="return confirm('Remove this document?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-5">
                <div class="flex items-center gap-2 mb-4">
                    <div class="w-7 h-7 rounded-xl bg-logo-blue/10 flex items-center justify-center">
                        <svg class="w-3.5 h-3.5 text-logo-blue" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                    </div>
                    <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Optional / Supporting
                        Documents</h2>
                </div>
                    ...
                    @foreach (array_diff_key(\App\Models\onlineBPLS\BplsDocument::TYPES, array_flip(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES)) as $type => $label)
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-2.5 min-w-0">
                                <div
                                    class="w-7 h-7 rounded-lg {{ $doc ? 'bg-white/60' : 'bg-lumot/20' }} flex items-center justify-center shrink-0">
                                    <svg class="w-3.5 h-3.5 {{ $doc ? 'text-logo-teal' : 'text-gray/40' }}"
                                ...
                                <div class="min-w-0">
                                    ...
                                    @if ($doc)
                                        <p class="text-[11px] text-gray truncate">{{ $doc->file_name }} &middot;
                                            {{ $doc->file_size_formatted }}</p>
                                    @else
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                @if ($doc && in_array($application->workflow_status, ['draft', 'returned']) && !$doc->isVerified())
                                    <form
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            class="text-xs font-bold text-red-400 hover:text-red-600 px-2 py-1 rounded-lg hover:bg-red-50 transition-colors">
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor" stroke-width="2.5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-5">
                ...
                @php
                    ...
                    $verifiedCount = collect($requiredTypes)
                        ->filter(fn($t) => $uploaded->has($t) && $uploaded->get($t)->isVerified())
                        ->count();
                    $rejectedCount = collect($requiredTypes)
                        ->filter(fn($t) => $uploaded->has($t) && $uploaded->get($t)->isRejected())
                        ->count();
                ...
                <div class="mb-3">
                    <div class="flex justify-between items-center mb-1.5">
                        ...
                        <span
                            class="text-xs font-extrabold text-logo-teal">{{ $uploadedCount }}/{{ $total }}</span>
                    </div>
                ...
                <div class="space-y-1.5">
                    @foreach ($requiredTypes as $type)
                        ...
                        <div class="flex items-center gap-2">
                            @if ($doc && $doc->isVerified())
                                <svg class="w-3.5 h-3.5 text-logo-green shrink-0" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="3">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                </svg>
                            @elseif($doc && $doc->isRejected())
                                <svg class="w-3.5 h-3.5 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="3">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            @elseif($doc)
                                <svg class="w-3.5 h-3.5 text-yellow-500 shrink-0" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <circle cx="12" cy="12" r="4" />
                                </svg>
                            @else
                                <svg class="w-3.5 h-3.5 text-lumot/60 shrink-0" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <circle cx="12" cy="12" r="4" />
                                </svg>
                            @endif
            ...
            @if (in_array($application->workflow_status, ['draft', 'returned']))
                <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-5">
                    ...
                    <p class="text-xs text-gray font-medium mb-4">
                        Once all required documents are uploaded, submit your application. Our back-office team will
                    ...
                    <div class="flex items-start gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-blue-600 font-medium">
                                Your documents are currently under review by our team. You will be notified once the
                                review is complete.
                            </p>
                        </div>
                    </div>
    ...
    <div x-data="uploadModal()" @open-upload.window="open($event.detail)" @keydown.escape.window="close()">
        <div x-show="show" class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10"
                ...
                <form action="{{ route('client.documents.upload', $application->id) }}" method="POST"
                    ...
                    <div class="border-2 border-dashed border-lumot/40 rounded-xl p-6 text-center hover:border-logo-teal transition-colors mb-4 relative"
                        ...
                        <input type="file" name="file" id="file-input"
                            ...
                            accept=".pdf,.jpg,.jpeg,.png,.webp" @change="handleFile($event.target.files[0])">

                        <template x-if="!file">
                            <div>
                                <svg class="w-8 h-8 text-lumot/60 mx-auto mb-2" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                </svg>
                                <p class="text-sm font-semibold text-gray">Drop file here or <span
                                        class="text-logo-teal font-bold">browse</span></p>
                        ...
                        <template x-if="file">
                            <div class="flex items-center gap-3 justify-center">
                                <svg class="w-6 h-6 text-logo-teal shrink-0" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                    ...
                    <div x-show="error"
                        class="mb-3 text-xs font-semibold text-red-500 bg-red-50 p-2.5 rounded-lg border border-red-100"
                        x-text="error"></div>
