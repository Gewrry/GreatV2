...
@php
    ...
    $requiredCount = count(array_intersect_key($existingDocsMap, array_flip(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES)));
@endphp
    ...
    <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm px-5 py-4 mb-6 overflow-x-auto">
        <div class="flex items-center min-w-max">
            ...
            <button type="button" @click="goTo(1)"
                ...
                <div class="flex items-center justify-center rounded-full shrink-0 transition-all duration-200 w-8 h-8"
                     :class="{
                        ...
                        'bg-logo-teal text-white shadow-md shadow-logo-teal/30 ring-2 ring-logo-teal/20 group-hover:ring-logo-teal/40': step !== 1
                     }">
                ...
                <div class="text-left">
                    ...
                    <p class="text-[10px] leading-tight"
                       :class="step === 1 ? 'text-logo-teal/60' : 'text-gray/30'">Owner &amp; business info</p>
                </div>
            </button>
            ...
            <button type="button" @click="goTo(2)"
                ...
                <div class="flex items-center justify-center rounded-full shrink-0 transition-all duration-200 w-8 h-8"
                     :class="{
                        'bg-logo-teal text-white shadow-md shadow-logo-teal/30 scale-110 ring-4 ring-logo-teal/15': step === 2,
                        'bg-logo-teal text-white shadow-md shadow-logo-teal/30 ring-2 ring-logo-teal/20 group-hover:ring-logo-teal/40': step !== 2
                     }">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="last_name" placeholder="e.g. Dela Cruz"
                                   ...
                                   class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-logo-teal/40 placeholder-gray/30 transition" required>
                        </div>
                        <div>
        ...
        <div x-show="step === 2"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                ...
                <div class="mb-5 p-3 bg-logo-teal/5 border border-logo-teal/20 rounded-xl flex items-start gap-2 mt-4">
                    ...
                    <p class="text-xs text-gray font-medium">All 3 required documents must be on file. Uploading a new file replaces the existing one. Max 5MB. Accepted: PDF, JPG, PNG.</p>
                </div>
                    ...
                    <div class="space-y-3">
                        @foreach(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES as $type)
                            ...
                            <div class="rounded-xl border p-4 transition-all duration-200"
                                 :class="hasDoc('{{ $type }}') ? 'border-logo-teal/40 bg-logo-teal/5' : 'border-lumot/30 bg-lumot/5'">
                                    ...
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <p class="text-[11px] truncate transition-colors"
                                               :class="hasDoc('{{ $type }}') ? 'text-logo-teal font-semibold' : 'text-gray/40'"
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        ...
                                        <label class="cursor-pointer">
                                            ...
                                            <span class="text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
                                                  :class="hasDoc('{{ $type }}') ? 'bg-logo-blue/10 text-logo-blue hover:bg-logo-blue/20' : 'bg-logo-teal text-white hover:bg-green shadow-sm shadow-logo-teal/20'"
                    ...
                    <div class="space-y-2">
                        @foreach(array_diff_key(\App\Models\onlineBPLS\BplsDocument::TYPES, array_flip(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES)) as $type => $label)
                            <div class="flex items-center justify-between gap-3 p-3 rounded-xl border transition-all"
                                 :class="hasDoc('{{ $type }}') ? 'border-logo-teal/30 bg-logo-teal/5' : 'border-lumot/20 bg-lumot/5'">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all"
                                         :class="hasDoc('{{ $type }}') ? 'bg-logo-teal/20' : 'bg-lumot/20'">
                                        <svg class="w-3.5 h-3.5" :class="hasDoc('{{ $type }}') ? 'text-logo-teal' : 'text-gray/30'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/>
                                        </svg>
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-xs font-semibold text-green truncate">{{ $label }}</p>
                                        <p class="text-[10px] truncate" :class="hasDoc('{{ $type }}') ? 'text-logo-teal' : 'text-gray/30'"
                                           x-text="docFiles['{{ $type }}'] ? docFiles['{{ $type }}'].name : (existingDocs['{{ $type }}'] ? existingDocs['{{ $type }}'].name + ' (saved)' : 'Not attached')"></p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <label class="cursor-pointer">
                                        ...
                                        <span class="text-xs font-bold px-2.5 py-1 rounded-lg transition-colors"
                                              :class="hasDoc('{{ $type }}') ? 'bg-logo-blue/10 text-logo-blue hover:bg-logo-blue/20' : 'bg-lumot/20 text-gray hover:bg-lumot/40'"
            ...
            <div class="flex justify-between">
                ...
                <button type="button"
                    ...
                    <span x-text="loading ? 'Saving...' : (requiredCount < 3 ? 'All 3 Required Docs Needed' : '{{ $application->workflow_status === 'returned' ? 'Save & Resubmit' : 'Save Changes' }}')"></span>
                </button>
            </div>
