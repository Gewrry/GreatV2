...
<body class="min-h-screen bg-gradient-to-br from-bluebody via-white to-blue/5">
    ...
    @php
        ...
        foreach ($application->documents as $doc) {
            $existingDocsMap[$doc->document_type] = [
                'name' => $doc->file_name,
                'size' => $doc->file_size,
                'path' => $doc->file_path,
            ];
        }
        $existingDocsJson = json_encode($existingDocsMap);
        $requiredCount = count(
            array_intersect_key($existingDocsMap, array_flip(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES)),
        );
    @endphp
    ...
    <div class="max-w-5xl mx-auto px-4 py-6" x-data="{
        ...
        hasDoc(type) { return !!(this.docFiles[type] || this.existingDocs[type]); },
        docLabel(type) {
            if (this.docFiles[type]) return this.docFiles[type].name + ' (' + this.formatSize(this.docFiles[type].size) + ')';
            if (this.existingDocs[type]) return this.existingDocs[type].name + ' (saved)';
        ...
        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm px-5 py-4 mb-6 overflow-x-auto">
            <div class="flex items-center min-w-max">
                ...
                <button type="button" @click="goTo(1)"
                    class="group flex items-center gap-2.5 focus:outline-none cursor-pointer">
                    <div class="flex items-center justify-center rounded-full shrink-0 transition-all duration-200 w-8 h-8"
                        :class="{
                            'bg-logo-teal text-white shadow-md shadow-logo-teal/30 scale-110 ring-4 ring-logo-teal/15': step ===
                                1,
                            'bg-logo-teal text-white shadow-md shadow-logo-teal/30 ring-2 ring-logo-teal/20 group-hover:ring-logo-teal/40': step !==
                                1
                        }">
                    ...
                    <div class="text-left">
                        <p class="text-xs font-bold leading-tight transition-colors group-hover:underline"
                            :class="step === 1 ? 'text-logo-teal' : 'text-green'">Fill Form</p>
                        <p class="text-[10px] leading-tight" :class="step === 1 ? 'text-logo-teal/60' : 'text-gray/30'">
                            Owner &amp; business info</p>
                    </div>
                </button>

                {{-- Connector --}}
                <div class="mx-3 shrink-0 h-px w-10 bg-logo-teal/60"></div>
                ...
                <button type="button" @click="goTo(2)"
                    ...
                    <div class="text-left">
                        <p class="text-xs font-bold leading-tight transition-colors group-hover:underline"
                            :class="step === 2 ? 'text-logo-teal' : 'text-green'">Upload Docs</p>
                        <p class="text-[10px] leading-tight"
                            :class="step === 2 ? 'text-logo-teal/60' : 'text-gray/30'">Required documents</p>
                    </div>
                </button>
                ...
                {{-- Locked admin steps --}}
                @foreach ($editLockedSteps as $i => $locked)
                    <div class="flex items-center gap-2.5 cursor-default">
                        <div class="w-7 h-7 rounded-full bg-lumot/15 flex items-center justify-center shrink-0">
                            @if ($i === count($editLockedSteps) - 1)
                                <svg class="w-3 h-3 text-gray/30" fill="none" viewBox="0 0 24 24"
        ...
        {{-- ══ FORM ═══════════════════════════════════════════════════════════ --}}
        <form action="{{ route('client.applications.update', $application->id) }}" method="POST" id="edit-form"
        ...
        ════════════════════════════════════════════════════════════════════ --}}
            <div x-show="step === 1" x-transition:enter="transition ease-out duration-200"
                ...
                x-data="{
                    sub: 1,
                    subMax: 4,
                    subNext() { if (this.sub < 4) this.sub++; },
                ...
                <div class="flex gap-1 mb-5 bg-white rounded-2xl p-1.5 shadow-sm border border-lumot/20">
                    @foreach ([['n' => 1, 'label' => 'Owner Info'], ['n' => 2, 'label' => 'Business Details'], ['n' => 3, 'label' => 'Business Address'], ['n' => 4, 'label' => 'Emergency Contact']] as $sub)
                        <button type="button" @click="subGoTo({{ $sub['n'] }})"
                            ...
                            <span
                                class="w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-extrabold"
                                :class="sub === {{ $sub['n'] }} ? 'bg-white/30' : sub > {{ $sub['n'] }} ?
                                    'bg-logo-green/30' : 'bg-gray/10'">
                ...
                <div x-show="sub === 1" x-transition:enter="transition ease-out duration-150"
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                        <div class="flex items-center gap-2 mb-5">
                            ...
                            <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Owner Information
                            </h2>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="citizenship"
                                    ...
                                    @foreach (['Filipino', 'Foreign National'] as $opt)
                                        <option value="{{ $opt }}"
                                            {{ old('citizenship', $application->owner->citizenship) === $opt ? 'selected' : '' }}>
                                            {{ $opt }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="civil_status"
                                    ...
                                    @foreach (['Single', 'Married', 'Widowed', 'Separated'] as $opt)
                                        <option value="{{ $opt }}"
                                            {{ old('civil_status', $application->owner->civil_status) === $opt ? 'selected' : '' }}>
                                            {{ $opt }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="gender"
                                    ...
                                    @foreach (['Male', 'Female', 'Prefer not to say'] as $opt)
                                        <option value="{{ $opt }}"
                                            {{ old('gender', $application->owner->gender) === $opt ? 'selected' : '' }}>
                                            {{ $opt }}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        'Street' => 'owner_street',
                                    ...
                                    @php $ownerField = str_replace('owner_', '', $field); @endphp
                                    <div class="{{ $lbl === 'Street' ? 'sm:col-span-2' : '' }}">
                                        <label
                                            class="block text-xs font-bold text-gray mb-1">{{ $lbl }}</label>
                ...
                <div x-show="sub === 2" x-transition:enter="transition ease-out duration-150"
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                        <div class="flex items-center gap-2 mb-5">
                            ...
                            <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Business Details
                            </h2>
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                <input type="date" name="date_of_application"
                                    value="{{ old('date_of_application', optional($application->business->date_of_application instanceof \Carbon\Carbon ? $application->business->date_of_application : \Carbon\Carbon::parse($application->business->date_of_application))->format('Y-m-d')) }}"
                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-logo-teal/40 text-gray">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="type_of_business"
                                    ...
                                    @foreach ($options['type_of_business'] as $opt)
                                        <option value="{{ $opt }}"
                                            {{ old('type_of_business', $application->business->type_of_business) === $opt ? 'selected' : '' }}>
                                            {{ $opt }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="mb-5">
                            <label class="block text-xs font-bold text-gray mb-2">Enjoying tax incentive from any
                                ...
                                <div>
                                    ...
                                    <select name="{{ $sel['name'] }}"
                                        ...
                                        @foreach ($options[$sel['name']] as $opt)
                                            <option value="{{ $opt }}"
                                                {{ old($sel['name'], $application->business->{$sel['name']}) === $opt ? 'selected' : '' }}>
                                                {{ $opt }}</option>
                                        @endforeach
                                    </select>
                                </div>
                ...
                <div x-show="sub === 3" x-transition:enter="transition ease-out duration-150"
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                        <div class="flex items-center gap-2 mb-5">
                            ...
                            <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Business Address
                            </h2>
                        </div>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            @foreach ([
        ...
        'Street' => ['field' => 'business_street', 'db' => 'street'],
                                ...
                                <div class="{{ $lbl === 'Street' ? 'sm:col-span-2' : '' }}">
                                    ...
                                    <input type="text" name="{{ $map['field'] }}"
                                        placeholder="{{ $lbl }}"
                ...
                <div x-show="sub === 4" x-transition:enter="transition ease-out duration-150"
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                        <div class="flex items-center gap-2 mb-5">
                            ...
                            <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Emergency Contact
                                Person</h2>
                        </div>
            ...
            <div x-show="step === 2" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-lumot/20 p-6 mb-4">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <div>
                            <h2 class="text-sm font-extrabold text-green uppercase tracking-wider">Documents</h2>
                            <p class="text-xs text-gray/50">Replace existing files or add new ones</p>
                        </div>
                    </div>

                    <div
                        class="mb-5 p-3 bg-logo-teal/5 border border-logo-teal/20 rounded-xl flex items-start gap-2 mt-4">
                        <svg class="w-4 h-4 text-logo-teal shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p class="text-xs text-gray font-medium">All 3 required documents must be on file. Uploading a
                            new file replaces the existing one. Max 5MB. Accepted: PDF, JPG, PNG.</p>
                    </div>
                    ...
                    {{-- Optional Documents --}}
                    <div>
                        <h3 class="text-xs font-extrabold text-green uppercase tracking-wider mb-3">Optional /
                        ...
                        <div class="space-y-2">
                            @foreach (array_diff_key(\App\Models\onlineBPLS\BplsDocument::TYPES, array_flip(\App\Models\onlineBPLS\BplsDocument::REQUIRED_TYPES)) as $type => $label)
                                <div class="flex items-center justify-between gap-3 p-3 rounded-xl border transition-all"
                                    :class="hasDoc('{{ $type }}') ? 'border-logo-teal/30 bg-logo-teal/5' :
                                        'border-lumot/20 bg-lumot/5'">
                                    <div class="flex items-center gap-2.5 min-w-0">
                                        <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all"
                                            :class="hasDoc('{{ $type }}') ? 'bg-logo-teal/20' : 'bg-lumot/20'">
                                            <svg class="w-3.5 h-3.5"
                                                :class="hasDoc('{{ $type }}') ? 'text-logo-teal' : 'text-gray/30'"
                                        ...
                                        <div class="min-w-0">
                                            <p class="text-xs font-semibold text-green truncate">{{ $label }}
                                            </p>
                                            <p class="text-[10px] truncate"
                                                :class="hasDoc('{{ $type }}') ? 'text-logo-teal' : 'text-gray/30'"
                                                x-text="docFiles['{{ $type }}'] ? docFiles['{{ $type }}'].name : (existingDocs['{{ $type }}'] ? existingDocs['{{ $type }}'].name + ' (saved)' : 'Not attached')">
                                            </p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <label class="cursor-pointer">
                                            ...
                                            <span class="text-xs font-bold px-2.5 py-1 rounded-lg transition-colors"
                                                ...
                                                x-text="hasDoc('{{ $type }}') ? 'Replace' : 'Attach'"></span>
                                        </label>
                                    </div>
                                </div>
                            @endforeach
