...
{{--
    ╔══════════════════════════════════════════════════════════════════╗
    ║  KEY FIX: All state (step, sub, docFiles, errors, etc.) lives   ║
    ║  in ONE root x-data object. Sub-step navigation buttons call    ║
    ║  methods from the same scope — no nested x-data on step 1.     ║
    ╚══════════════════════════════════════════════════════════════════╝
--}}
<div class="max-w-4xl mx-auto px-4 sm:px-6 py-8" x-data="{
    ...
    next()   { if (this.step < 2) { this.step++; if (this.step > this.maxReached) this.maxReached = this.step; } },
    ...
    beneficiaryFlags: {
        @foreach($benefits as $benefit)
            {{ $benefit->field_key }}: {{ old($benefit->field_key, ($renewal && $renewal->owner && str_starts_with($benefit->field_key, 'is_')) ? ($renewal->owner->{$benefit->field_key} ? 'true' : 'false') : 'false') }},
        @endforeach
    },
    ...
    get requiredCount() { return this.dynamicRequiredTypes.filter(t => !!this.docFiles[t]).length; },
    get totalRequired()  { return this.dynamicRequiredTypes.length; },
    ...
    {{-- ── Flash / Errors ──────────────────────────────────────────── --}}
    @if (session('success'))
        <div class="mb-6 flex items-start gap-3 p-4 bg-logo-green/8 border border-logo-green/25 rounded-2xl text-sm text-green font-semibold">
    ...
    <div class="mb-8">
        ...
        <div class="flex items-end justify-between gap-4">
            ...
            <span class="text-xs font-bold text-logo-teal bg-logo-teal/8 border border-logo-teal/15 px-3 py-1.5 rounded-full whitespace-nowrap">
                BPLS {{ date('Y') }}
            </span>
        </div>
    </div>
        ...
        <div class="flex items-center min-w-max gap-0">
            ...
            <button type="button" @click="goTo(1)" class="flex items-center gap-2 group focus:outline-none"
                ...
                <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-200 shrink-0"
                    ...
                    <template x-if="step === 1 || maxReached <= 1"><span>1</span></template>
                </div>
                <div class="text-left">
            ...
            <button type="button" @click="goTo(2)" class="flex items-center gap-2 group focus:outline-none"
                ...
                <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-200 shrink-0"
                    :class="{
                        'bg-green text-white ring-4 ring-green/10': step === 2,
                        'bg-logo-teal text-white': step !== 2 && maxReached > 2,
                        'bg-lumot/20 text-gray/40': maxReached < 2,
                        'bg-lumot/30 text-gray/50': maxReached === 2 && step !== 2
                    }">
                    ...
                    <template x-if="!(step !== 2 && maxReached > 2)"><span>2</span></template>
                </div>
                <div class="text-left">
    ...
    <form action="{{ route('client.apply.store') }}" method="POST" id="bpls-form" enctype="multipart/form-data">
        ...
        {{-- ═══════════════════════════════════════════════════════════
             STEP 1 — Fill Form
             NOTE: NO nested x-data here. Uses root scope directly.
        ═══════════════════════════════════════════════════════════════ --}}
        <div x-show="step === 1"
                ...
                <template x-for="i in 4" :key="i">
                    <button type="button"
                        ...
                        :class="sub === i
                            ...
                            : sub > i
                                ? 'bg-logo-green/20 text-green'
                                : 'text-gray hover:bg-lumot/20'"
                        class="flex-1 py-2 px-3 rounded-xl text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed">
                        <span x-text="i"
                            class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-extrabold"
                            :class="sub === i ? 'bg-white/30' : sub > i ? 'bg-logo-green/30' : 'bg-gray/10'"></span>
                        <span x-text="['Owner Info','Business Details','Business Address','Emergency Contact'][i-1]" class="hidden sm:block"></span>
                        <span x-text="['Owner','Business','Address','Contact'][i-1]" class="sm:hidden"></span>
                    </button>
                </template>
            </div>
            ...
            <div x-show="sub === 1"
                ...
                <div class="bg-white border border-lumot/15 rounded-2xl p-5 sm:p-7 mb-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Last Name <span class="text-red-400">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">First Name <span class="text-red-400">*</span></label>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Citizenship <span class="text-red-400">*</span></label>
                            <select name="citizenship" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Civil Status <span class="text-red-400">*</span></label>
                            <select name="civil_status" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                                <option value="">Select</option>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Gender <span class="text-red-400">*</span></label>
                            <select name="gender" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                                <option value="">Select</option>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Mobile No. <span class="text-red-400">*</span></label>
                    ...
                    {{-- Owner Address --}}
                    <div x-data="addressPicker('owner')"
                         x-init="init(
                            '{{ old('owner_region', $renewal?->owner?->region ?? '') }}',
                        ...
                        <h3 class="text-xs font-bold text-green/80 uppercase tracking-wider mb-3">Owner's Address</h3>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Region <span class="text-red-400">*</span></label>
                                ...
                                <p x-show="loadingProvinces" class="text-[10px] text-logo-teal mt-1 animate-pulse">Loading…</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Province</label>
                                <select name="owner_province" @change="onProvinceChange($event)" :disabled="!provinces.length"
                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all disabled:opacity-40">
                                ...
                                <p x-show="loadingMunicipalities" class="text-[10px] text-logo-teal mt-1 animate-pulse">Loading…</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Municipality / City <span class="text-red-400">*</span></label>
                                <select name="owner_municipality" @change="onMunicipalityChange($event)" :disabled="!municipalities.length"
                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all disabled:opacity-40">
                                ...
                                <p x-show="loadingBarangays" class="text-[10px] text-logo-teal mt-1 animate-pulse">Loading…</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Barangay <span class="text-red-400">*</span></label>
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Street / Purok / Sitio</label>
                ...
                <div class="flex justify-end">
                    <button type="button" @click="validateSub(1) && subNext()"
                        class="inline-flex items-center gap-2 px-8 py-3 bg-logo-teal text-white text-sm font-bold rounded-xl hover:bg-logo-teal/90 transition-all shadow-md shadow-logo-teal/20">
                        Next Step
            ...
            <div x-show="sub === 2"
                ...
                <div class="bg-white border border-lumot/15 rounded-2xl p-5 sm:p-7 mb-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Business Name <span class="text-red-400">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Trade Name / Franchise</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                        <div class="sm:col-span-2">
                            <label class="block text-xs font-bold text-green/80 mb-1.5">DTI/SEC/CDA Registration No.</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Type of Business <span class="text-red-400">*</span></label>
                            <select name="type_of_business" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                    ...
                    <div class="p-4 bg-yellow/5 border border-yellow/20 rounded-xl mb-5">
                        <p class="text-xs font-bold text-green/70 uppercase tracking-wider mb-3">Amendment (optional)</p>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">From</label>
                                <select name="amendment_from" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                                    <option value="">Select</option>
                            ...
                            <div>
                                ...
                                <select name="amendment_to" class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all">
                                    <option value="">Select</option>
                    ...
                    {{-- Tax incentive --}}
                    <div class="mb-5">
                        <label class="block text-xs font-bold text-green/80 mb-2.5">Enjoying tax incentive from any Government Entity?</label>
                        <div class="flex gap-4">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="tax_incentive" value="0"
                                    ...
                                    class="text-logo-teal focus:ring-logo-teal/30 w-4 h-4">
                                <span class="text-sm font-semibold text-green/70">No</span>
                            </label>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($benefits as $benefit)
                                <label class="cursor-pointer">
                                    <input type="checkbox" name="{{ $benefit->field_key }}"
                                        class="peer hidden"
                                        x-model="beneficiaryFlags.{{ $benefit->field_key }}"
                                        {{ old($benefit->field_key) ? 'checked' : '' }}>
                    ...
                    <div class="pt-5 border-t border-lumot/15 mb-5">
                        <p class="text-xs font-bold text-green/70 uppercase tracking-wider mb-3">Business Classification</p>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Business Nature <span class="text-red-400">*</span></label>
                                <input type="text" name="business_nature"
                                    placeholder="e.g. Eatery, Trading, Manufacturing…"
                                    value="{{ old('business_nature', $renewal?->business?->business_nature ?? '') }}"
                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 placeholder-gray/25 bg-white transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Capital Investment (₱) <span class="text-red-400">*</span></label>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
                            @foreach ([
                                ['name' => 'business_organization', 'label' => 'Organization'],
                                ['name' => 'business_area_type',    'label' => 'Area Type'],
                                ['name' => 'business_scale',        'label' => 'Scale'],
                                ['name' => 'business_sector',       'label' => 'Sector'],
                                ['name' => 'zone',                  'label' => 'Zone'],
                                ['name' => 'occupancy',             'label' => 'Occupancy'],
                            ] as $sel)
                                <div>
                                    <label class="block text-xs font-bold text-green/80 mb-1.5">{{ $sel['label'] }}</label>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-green/80 mb-1.5">Total Employees</span></label>
                ...
                <div class="flex justify-between items-center mt-6">
                    <button type="button" @click="subPrev(); clearErrors()"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-white text-gray text-sm font-bold rounded-xl border border-gray/20 hover:bg-gray/5 transition-all">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                        </svg>
                        Back
            ...
            <div x-show="sub === 3"
                ...
                <div class="bg-white border border-lumot/15 rounded-2xl p-5 sm:p-7 mb-5 shadow-sm">
                    ...
                    {{-- addressPicker here is a LOCAL nested x-data — this is fine because it
                         only manages its own dropdown state; the sub navigation buttons below
                         call the ROOT scope methods (subPrev / validateSub / subNext). --}}
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4"
                         x-data="addressPicker('business')"
                         x-init="init(
                            '{{ old('business_region', $renewal?->business?->region ?? '') }}',
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Province</label>
                            <select name="business_province" @change="onProvinceChange($event)" :disabled="!provinces.length"
                                class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all disabled:opacity-40">
                                <option value="">Select Province</option>
                                <template x-for="p in provinces" :key="p.code">
                                    <option :value="p.name" :data-code="p.code" :selected="p.name === selectedProvinceName" x-text="p.name"></option>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-green/80 mb-1.5">Municipality / City <span class="text-red-400">*</span></label>
                            <select name="business_municipality" @change="onMunicipalityChange($event)" :disabled="!municipalities.length"
                                class="w-full text-sm border border-lumot/30 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-logo-teal/30 focus:border-logo-teal/60 text-gray bg-white transition-all disabled:opacity-40">
                                <option value="">Select Municipality</option>
                                <template x-for="m in municipalities" :key="m.code">
                                    <option :value="m.name" :data-code="m.code" :selected="m.name === selectedMunicipalityName" x-text="m.name"></option>
        ...
        <div x-show="step === 2"
            ...
            <div class="bg-white border border-lumot/15 rounded-2xl p-5 sm:p-7 mb-5 shadow-sm">
                ...
                {{-- Info banner --}}
                <div class="mb-5 p-3 bg-logo-teal/5 border border-logo-teal/20 rounded-xl flex items-start gap-2">
                                    ...
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <p class="text-[11px] truncate transition-colors"
                                               :class="docFiles[type] ? 'text-logo-teal font-semibold' : 'text-gray/40'">
                ...
                {{-- Optional Documents --}}
                <div class="pt-5 border-t border-lumot/15 mb-6">
                ...
                {{-- Progress bar --}}
                <div class="mb-5 pt-4 border-t border-lumot/20">
                    ...
                    <div class="w-full h-2 bg-lumot/30 rounded-full overflow-hidden">
                        <div class="h-full bg-logo-teal rounded-full transition-all duration-500"
                             :style="'width: ' + (totalRequired > 0 ? (requiredCount / totalRequired * 100) : 0) + '%'"></div>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-2.5 p-4 bg-blue/5 border border-blue/10 rounded-xl mb-5">
                    <svg class="w-4 h-4 text-logo-blue/60 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<script>
    ...
    function addressPicker(prefix) {
        return {
            ...
            selectedRegionCode: '', selectedRegionName: '',
            ...
            async onMunicipalityChange(event) {
                ...
                this.barangays = []; this.selectedBarangayName = '';
            ...
            _fixEncoding(str) {
                if (!str) return str;
                const map = {
                    'ÃŸ':'ß','Ã ':'à','Ã¢':'â','Ã£':'ã','Ã¤':'ä','Ã¥':'å','Ã¦':'æ',
                    'Ã§':'ç','Ã¨':'è','Ãª':'ê','Ã«':'ë','Ã¬':'ì','Ã®':'î','Ã¯':'ï',
                    'Ã°':'ð','Ã²':'ò','Ã´':'ô','Ãµ':'õ','Ã¶':'ö','Ã·':'÷','Ã¸':'ø',
                    'Ã¹':'ù','Ã»':'û','Ã½':'ý','Ã¾':'þ','Ã¿':'ÿ'
                };
                let fixed = str;
                for (const [k, v] of Object.entries(map)) fixed = fixed.replace(new RegExp(k, 'g'), v);
                return fixed;
            }
        };
    }
