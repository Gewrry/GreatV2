{{-- resources/views/client/applications/receipt.blade.php --}}
{{--
...
<head>
    ...
    @php
        ...
        // Dynamic mode installments based on what the $entry (application or business) reports
        $modeOfPayment = $entry->mode_of_payment ?? 'annual';
        $modeCount = match ($modeOfPayment) {
            'annual' => 1,
            'semi_annual' => 2,
            default => 4,
        };
    ...
    <div class="no-print-btn">
        ...
        @if(Auth::guard('client')->check())
             @php
                ...
                } elseif (isset($entry->bpls_application_id)) {
                    $applicationId = $entry->bpls_application_id;
                }
            @endphp

            @if($applicationId)
                <a href="{{ route('client.applications.show', $applicationId) }}">← Back to Application</a>
            @else
    ...
    <div class="receipt-wrap">
        ...
        @if ($showAmountInWords)
            <div class="words-row">
                ...
                <div class="value">
                    @php
                        if (!function_exists('numToWordsReceiptOnline')) {
                            function numToWordsReceiptOnline(float $amount): string
                            {
                        ...
                        echo numToWordsReceiptOnline((float) $payment->total_collected);
                    @endphp
                </div>
            </div>
        @endif
