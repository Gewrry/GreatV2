        ...
        function validateSpatialSafeguards(layer) {
            ...
            if (modalOptions.parent_boundary) {
                try {
                    ...
                    if (pctInside < 0.99) {
                        alert(`ERROR: The parcel boundary must be strictly WITHIN the parent property boundary. \n\nCurrently, ${((1 - pctInside) * 100).toFixed(1)}% of your mapping is outside the parent boundary. Please adjust your points.`);
                        return false;
                    }
                } catch (err) {
            ...
            setTimeout(() => {
                try {
                    ...
                    if ($('#modal-preview-toggle').is(':checked') && currentStrategy !== 'FILL') {
                        ...
                        $('#autoplot-btn-label').text(`Apply (${computedArea.toFixed(1)} SQM, Δ${diff.toFixed(2)})`);
                    } else {
