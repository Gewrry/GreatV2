<x-admin.app>
    ...
    <div class="p-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-widest mb-6">
                ...
                <span class="text-gray-600">Cancelled TD List (Archive)</span>
            </div>

            <!-- Header -->
            <div class="flex justify-between items-end mb-8">
                <div>
                    <h1 class="text-3xl font-black text-gray-900 tracking-tight">Cancelled/Superseded TDs</h1>
                    <p class="text-gray-500 mt-2 font-medium">Archive of properties that have been cancelled or superseded by new records.</p>
                </div>
                <div class="flex gap-3">
                    <form action="{{ route('rpt.reports.cancelled_list.export.pdf') }}" method="GET" id="export-form">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-8">
                    <table id="cancelled-report-table" class="w-full text-left">
                        <thead>
                            <tr class="border-b border-gray-100">
                                <th class="pb-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">ARPN / TD No.</th>
                                <th class="pb-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Previous Owner</th>
                                ...
                                <th class="pb-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Last Assessed</th>
    ...
    $(document).ready(function() {
        const table = $('#cancelled-report-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('rpt.reports.cancelled_list') }}",
                data: function(d) {
            ...
            columns: [
                    ...
                    render: function(data, type, row) {
                        ...
                        return `
                            <div class="flex flex-col">
                                ...
                                <span class="text-[10px] font-bold text-red-600 uppercase tracking-tight">${row.td_no}</span>
                            </div>
                        `;
                    }
                },
                { data: 'owner_names', name: 'owner_names', render: data => `<span class="text-xs font-bold text-gray-700">${data}</span>` },
                ...
                { 
                    data: 'statt',
                    render: status => {
                        let cls = status === 'CANCELLED' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-amber-100 text-amber-700 border-amber-200';
