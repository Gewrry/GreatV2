{{-- Add Machinery Modal --}}
<div id="addMachModal" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div class="px-6 py-4 bg-purple-600 text-white flex justify-between items-center">
            <h3 class="font-bold text-lg leading-none tracking-tight">Add Machinery Appraisal</h3>
            <button onclick="document.getElementById('addMachModal').classList.add('hidden')" class="text-purple-100 hover:text-white"><i class="fas fa-times"></i></button>
        </div>
        <form action="{{ route('rpt.faas.machinery.store', $faas) }}" method="POST" class="p-6 space-y-4 machinery-calc-container">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <select name="faas_land_id" class="w-full border-gray-200 border rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                ...
                <div class="md:col-span-2">
                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Classification *</label>
                    <select name="rpta_actual_use_id" required class="w-full border-gray-200 border rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                ...
                <div><label class="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Acquisition Year</label><input type="number" name="year_acquired" min="1950" max="{{ date('Y') }}" required value="{{ date('Y') }}" class="w-full border-gray-200 border rounded-xl px-4 py-2.5 text-sm"></div>
                ...
                <div class="md:col-span-2 bg-purple-50/50 p-4 rounded-xl border border-purple-100 mt-2">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-[10px] font-bold text-purple-800 uppercase tracking-widest">Auto Valuation</span>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-50 uppercase tracking-widest text-[10px] font-bold">
                <button type="button" onclick="document.getElementById('addMachModal').classList.add('hidden')" class="px-5 py-2.5 border border-gray-200 rounded-xl text-gray-500 hover:bg-gray-50 transition-all">Cancel</button>
                <button type="submit" class="bg-purple-600 text-white px-8 py-2.5 rounded-xl hover:bg-purple-700 shadow-lg shadow-purple-100 transition-all">Add Machinery</button>
            </div>
        </form>
    </div>
</div>
