{{-- Add Land Modal --}}
<div id="addLandModal" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form action="{{ route('rpt.faas.land.store', $faas) }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label class="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Market Adjustments (₱)</label><input type="number" name="market_value_adjustments" step="0.01" value="0" class="w-full border-gray-200 border rounded-xl px-4 py-2.5 text-sm"></div>
