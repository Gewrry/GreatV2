{{-- resources/views/modules/bpls/reports/compliance_monitoring_daterange.blade.php --}}
<x-admin.app>
    <div class="py-2">
        <div class="w-full px-2 sm:px-4 lg:px-6">
            ...
            <div class="min-h-screen bg-gradient-to-br from-bluebody via-white to-blue/5 p-3 sm:p-4"
                x-data="complianceDateRange()" x-init="init()">
                ...
                <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                        <h1 class="text-xl sm:text-2xl font-extrabold text-green tracking-tight">Compliance Monitoring
                            Report</h1>
                        <p class="text-gray text-xs sm:text-sm mt-0.5">Track payment compliance over a custom date
                            range</p>
                    </div>
                    <span
                ...
                <div class="flex flex-col lg:flex-row gap-4 items-start">
                        ...
                        <div
                            ...
                            <div class="p-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
                                    ...
                                    <div>
                                        <label
                                            class="block text-[10px] font-extrabold text-gray/60 uppercase mb-1.5">Compliance</label>
                                    ...
                                    <div>
                                        <label
                                            class="block text-[10px] font-extrabold text-gray/60 uppercase mb-1.5">Fund
                                            Code</label>
                                        <input type="text" x-model="filters.fund_code" placeholder="e.g. 100"
                                            class="w-full text-xs border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40 placeholder-gray/30">
                                    </div>
                    ...
                    <div class="flex-1 min-w-0 w-full">
                        ...
                        <div x-show="!generated && !generating" x-cloak
                            ...
                            <p class="text-xs text-gray/50 mt-1">Set a date range and click Generate Report.</p>
                        </div>
                        ...
                        <div x-show="generated && !generating" x-cloak
                            ...
                            <div x-show="results.length > 0"
                                ...
                                <div class="px-3 sm:px-4 py-2.5 text-center">
                                    <p class="text-[10px] text-gray/50 uppercase font-bold">Total Payments</p>
                                    <p class="text-lg font-extrabold text-green" x-text="results.length"></p>
                                </div>
                                <div class="px-3 sm:px-4 py-2.5 text-center">
                                    <p class="text-[10px] text-gray/50 uppercase font-bold">Total Surcharges</p>
                                    <p class="text-sm font-extrabold text-red-500"
                                        x-text="'₱' + results.reduce((s,r)=>s+(parseFloat(r.surcharges)||0),0).toLocaleString('en-PH',{minimumFractionDigits:2})">
                                    </p>
                                </div>
                                <div class="px-3 sm:px-4 py-2.5 text-center">
                                    <p class="text-[10px] text-gray/50 uppercase font-bold">Total Discounts</p>
                                    <p class="text-sm font-extrabold text-logo-green"
                                        x-text="'₱' + results.reduce((s,r)=>s+(parseFloat(r.discount)||0),0).toLocaleString('en-PH',{minimumFractionDigits:2})">
                                    </p>
                                </div>
                                <div class="px-3 sm:px-4 py-2.5 text-center">
                            ...
                            <div x-show="results.length > 0"
                                ...
                                <table class="w-full text-xs border-collapse" style="min-width: 950px;">
                                    <thead>
                                    ...
                                    <tfoot>
                                        <tr class="bg-logo-teal/5 border-t-2 border-logo-teal/20">
                                            ...
                                            <td class="px-3 py-2.5 text-right font-mono text-gray/50 whitespace-nowrap text-xs"
                                                x-text="'₱' + results.reduce((s,r)=>s+(parseFloat(r.amount_paid)||0),0).toLocaleString('en-PH',{minimumFractionDigits:2})">
                                            </td>
                                            <td class="px-3 py-2.5 text-right font-mono text-red-400 whitespace-nowrap text-xs"
                                                x-text="'₱' + results.reduce((s,r)=>s+(parseFloat(r.surcharges)||0),0).toLocaleString('en-PH',{minimumFractionDigits:2})">
                                            </td>
                                            <td class="px-3 py-2.5 text-right font-mono text-logo-green whitespace-nowrap text-xs"
                                                x-text="'₱' + results.reduce((s,r)=>s+(parseFloat(r.discount)||0),0).toLocaleString('en-PH',{minimumFractionDigits:2})">
                                            </td>
                ...
                return {
                    ...
                    async generate() {
                        ...
                        try {
                            ...
                            const res = await fetch(
                                `{{ route('bpls.reports.compliance.daterange.data') }}?${params}`, {
                                    headers: {
                    ...
                    exportCsv() {
                        const headers = ['#', 'Business Name', 'Trade Name', 'Owner', 'Barangay', 'Year',
                            'OR Number', 'Quarters Paid', 'Payment Date', 'Method', 'Fund Code',
                            'Amount Paid', 'Surcharges', 'Discount', 'Total Collected'
                        ];
                        const rows = this.results.map((r, i) => [
                            i + 1,
                            `"${r.business_name ?? ''}"`, `"${r.trade_name ?? ''}"`,
                            `"${(r.last_name ?? '') + ', ' + (r.first_name ?? '')}"`,
                            `"${r.business_barangay ?? ''}"`, r.payment_year,
                            `"${r.or_number ?? ''}"`, `"${r.quarters_paid ?? ''}"`,
                            `"${r.payment_date ?? ''}"`, `"${r.payment_method ?? ''}"`,
                            `"${r.fund_code ?? ''}"`,
                            r.amount_paid ?? 0, r.surcharges ?? 0, r.discount ?? 0, r.total_collected ?? 0,
                        ]);
                        ...
                        a.href = url;
                        a.download = `compliance_daterange_${Date.now()}.csv`;
                        a.click();
