        ...
        <table class="w-full text-left border-collapse min-w-[1200px]">
            <thead>
                <tr class="bg-white border-b border-lumot/10">
                    <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40">
                        Business Info</th>
                    <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40">
                        Taxpayer / Contact</th>
                    <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40">Nature
                        & Scale</th>
                    <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40">
                        Barangay</th>
                    <th
                        class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40 text-center">
                        Status</th>
                    <th
                        class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-green opacity-40 text-right">
                        Investment / Due</th>
                </tr>
            </thead>
                    ...
                    <tr class="hover:bg-bluebody/10 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-10 h-10 rounded-xl bg-bluebody/50 flex items-center justify-center text-logo-teal font-black text-xs shrink-0 border border-lumot/10 group-hover:bg-logo-teal group-hover:text-white transition-all">
                                    {{ substr($bus->business_name, 0, 2) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-green leading-tight truncate">
                                        {{ $bus->business_name }}</p>
                                    <p class="text-[10px] text-gray/50 mt-1 uppercase tracking-tighter">
                                        {{ $bus->trade_name ?? 'No Trade Name' }}</p>
                                    <div class="flex items-center gap-2 mt-1.5">
                                        <span
                                            class="text-[9px] font-black bg-logo-teal/10 text-logo-teal px-2 py-0.5 rounded uppercase tracking-wider">{{ $bus->type_of_business ?? 'Retail' }}</span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <p class="text-xs font-bold text-green tracking-tight">{{ $bus->last_name }},
                                {{ $bus->first_name }}</p>
                        ...
                        <td class="px-6 py-4">
                            <div class="max-w-[150px]">
                                <p class="text-xs font-bold text-green truncate">
                                    {{ $bus->business_nature ?? 'General Merchant' }}</p>
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-black uppercase {{ str_contains($bus->business_scale, 'Large') ? 'bg-indigo-50 text-indigo-600' : 'bg-logo-green/10 text-logo-green' }} mt-1 border border-black/5">
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @endphp
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-lg {{ $statusStyles }}">
                                {{ str_replace('_', ' ', $bus->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex flex-col items-end">
                                <p class="text-xs font-black text-green">
                                    ₱{{ number_format($bus->capital_investment, 2) }}</p>
                                <p class="text-[10px] font-bold text-logo-teal mt-1">Due:
                                    ₱{{ number_format($bus->total_due, 2) }}</p>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-4">
                                <div
                                    class="w-16 h-16 bg-bluebody/30 rounded-full flex items-center justify-center text-gray/20">
                                    <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path
                                            d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0V9a2 2 0 00-2-2H6a2 2 0 00-2 2v10m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2" />
                                    </svg>
                                </div>
                                <div>
