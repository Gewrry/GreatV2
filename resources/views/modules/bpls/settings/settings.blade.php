        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="min-h-screen bg-gradient-to-br from-bluebody via-white to-blue/5 p-4">
                ...
                <div id="discounts" class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-6">
                    <div class="bg-green text-white text-center py-2.5">
                        <p class="text-xs font-extrabold tracking-wide uppercase">Discounts & Surcharges</p>
                    </div>
                    ...
                    <div class="p-5">
                        <form action="{{ route('bpls.settings.update-discount') }}" method="POST">
                            ...
                            <div class="mb-6">
                                <h3 class="text-sm font-bold text-gray mb-3 flex items-center gap-2">
                                ...
                                <div class="mb-4">
                                    ...
                                    <p class="text-xs text-gray/50 mt-1 ml-6">Allow discounts for payments made before due date</p>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Quarterly Discount Rate (%)</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Semi-Annual Discount Rate (%)</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Annual Discount Rate (%)</label>
                                ...
                                <div class="mb-4">
                                    <label class="block text-xs font-bold text-gray mb-1">Days Before Due Date to Qualify</label>
                                    <input type="number" name="advance_discount_days_before"
                                        value="{{ $settings['advance_discount_days_before']->value ?? 10 }}"
                                        min="1" max="365"
                                        class="w-full md:w-64 text-sm border border-lumot/30 rounded-xl px-3 py-2
                                               focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    <p class="text-xs text-gray/50 mt-1">Number of days before due date to qualify for discount (Default: 10 days)</p>
                                </div>
                            ...
                            {{-- ══════════════════════════════════════════════════════════
                                 ── PWD / Senior / Solo Parent / 4Ps Discounts (NEW) ──
                            ══════════════════════════════════════════════════════════ --}}
                            <div class="mb-6 pt-5 border-t border-lumot/20">
                                <h3 class="text-sm font-bold text-gray mb-1 flex items-center gap-2">
                                    <span class="w-1.5 h-4 bg-purple-400 rounded-full inline-block"></span>
                                    Beneficiary Discounts
                                </h3>
                                <p class="text-xs text-gray/50 mb-4 ml-3.5">
                                    These discounts are automatically applied during payment when the business owner
                                    belongs to a qualified beneficiary group (PWD, Senior Citizen, Solo Parent, 4Ps).
                                    Discounts are applied on top of assessed fees and stack with the advance discount.
                                </p>

                                {{-- Enable toggle --}}
                                <div class="mb-4">
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="beneficiary_discount_enabled" value="1"
                                            {{ ($settings['beneficiary_discount_enabled']->value ?? '1') == '1' ? 'checked' : '' }}
                                            class="rounded border-lumot/30 text-logo-teal focus:ring-logo-teal/20">
                                        <span class="text-sm font-semibold text-gray">Enable Beneficiary Discounts</span>
                                    </label>
                                    <p class="text-xs text-gray/50 mt-1 ml-6">Master toggle — disabling this skips all beneficiary discount computations</p>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="p-4 bg-purple-50 border border-purple-100 rounded-xl">
                                        <div class="flex items-center gap-2 mb-3">
                                            ...
                                            <p class="text-xs font-extrabold uppercase tracking-wide text-purple-700">PWD (Person with Disability)</p>
                                        </div>
                                        <div class="space-y-3">
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Discount Rate (%)</label>
                                                <input type="number" name="pwd_discount_rate"
                                                    value="{{ $settings['pwd_discount_rate']->value ?? 20 }}"
                                                    step="0.1" min="0" max="100"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-purple-300">
                                                <p class="text-xs text-gray/50 mt-1">Applied when owner's <code>is_pwd</code> is checked. Default: 20%</p>
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Apply To</label>
                                                <select name="pwd_discount_apply_to"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-purple-300">
                                                    <option value="total" {{ ($settings['pwd_discount_apply_to']->value ?? 'total') === 'total' ? 'selected' : '' }}>Total Amount Due</option>
                                                    <option value="permit_only" {{ ($settings['pwd_discount_apply_to']->value ?? '') === 'permit_only' ? 'selected' : '' }}>Mayor's Permit Fee Only</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-amber-50 border border-amber-100 rounded-xl">
                                        <div class="flex items-center gap-2 mb-3">
                                            <span class="text-base">👴</span>
                                            <p class="text-xs font-extrabold uppercase tracking-wide text-amber-700">Senior Citizen</p>
                                        </div>
                                        <div class="space-y-3">
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Discount Rate (%)</label>
                                                <input type="number" name="senior_discount_rate"
                                                    value="{{ $settings['senior_discount_rate']->value ?? 20 }}"
                                                    step="0.1" min="0" max="100"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-amber-300">
                                                <p class="text-xs text-gray/50 mt-1">Applied when owner's <code>is_senior</code> is checked. Default: 20%</p>
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Apply To</label>
                                                <select name="senior_discount_apply_to"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-amber-300">
                                                    <option value="total" {{ ($settings['senior_discount_apply_to']->value ?? 'total') === 'total' ? 'selected' : '' }}>Total Amount Due</option>
                                                    <option value="permit_only" {{ ($settings['senior_discount_apply_to']->value ?? '') === 'permit_only' ? 'selected' : '' }}>Mayor's Permit Fee Only</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-pink-50 border border-pink-100 rounded-xl">
                                        <div class="flex items-center gap-2 mb-3">
                                            <span class="text-base">👩‍👧</span>
                                            <p class="text-xs font-extrabold uppercase tracking-wide text-pink-700">Solo Parent</p>
                                        </div>
                                        <div class="space-y-3">
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Discount Rate (%)</label>
                                                <input type="number" name="solo_parent_discount_rate"
                                                    value="{{ $settings['solo_parent_discount_rate']->value ?? 10 }}"
                                                    step="0.1" min="0" max="100"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-pink-300">
                                                <p class="text-xs text-gray/50 mt-1">Applied when owner's <code>is_solo_parent</code> is checked. Default: 10%</p>
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Apply To</label>
                                                <select name="solo_parent_discount_apply_to"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-pink-300">
                                                    <option value="total" {{ ($settings['solo_parent_discount_apply_to']->value ?? 'total') === 'total' ? 'selected' : '' }}>Total Amount Due</option>
                                                    <option value="permit_only" {{ ($settings['solo_parent_discount_apply_to']->value ?? '') === 'permit_only' ? 'selected' : '' }}>Mayor's Permit Fee Only</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-green-50 border border-green-100 rounded-xl">
                                        <div class="flex items-center gap-2 mb-3">
                                            <span class="text-base">🏠</span>
                                            <p class="text-xs font-extrabold uppercase tracking-wide text-green-700">4Ps Beneficiary</p>
                                        </div>
                                        <div class="space-y-3">
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Discount Rate (%)</label>
                                                <input type="number" name="fourps_discount_rate"
                                                    value="{{ $settings['fourps_discount_rate']->value ?? 10 }}"
                                                    step="0.1" min="0" max="100"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-green-300">
                                                <p class="text-xs text-gray/50 mt-1">Applied when owner's <code>is_4ps</code> is checked. Default: 10%</p>
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold text-gray mb-1">Apply To</label>
                                                <select name="fourps_discount_apply_to"
                                                    class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                           focus:outline-none focus:ring-2 focus:ring-green-300">
                                                    <option value="total" {{ ($settings['fourps_discount_apply_to']->value ?? 'total') === 'total' ? 'selected' : '' }}>Total Amount Due</option>
                                                    <option value="permit_only" {{ ($settings['fourps_discount_apply_to']->value ?? '') === 'permit_only' ? 'selected' : '' }}>Mayor's Permit Fee Only</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                                    <p class="text-xs font-semibold text-yellow-800 mb-2">⚠️ Stacking Rule</p>
                                    <div class="flex items-start gap-3">
                                        <div class="flex-1">
                                            <label class="flex items-center gap-2 mb-1">
                                                <input type="radio" name="beneficiary_discount_stack" value="highest_only"
                                                    {{ ($settings['beneficiary_discount_stack']->value ?? 'highest_only') === 'highest_only' ? 'checked' : '' }}
                                                    class="text-logo-teal focus:ring-logo-teal/20">
                                                <span class="text-xs font-semibold text-gray">Apply highest discount only</span>
                                            </label>
                                            <p class="text-xs text-gray/50 ml-5">If owner qualifies for multiple groups, only the highest rate is applied.</p>
                                        </div>
                                        <div class="flex-1">
                                            <label class="flex items-center gap-2 mb-1">
                                                <input type="radio" name="beneficiary_discount_stack" value="additive"
                                                    {{ ($settings['beneficiary_discount_stack']->value ?? '') === 'additive' ? 'checked' : '' }}
                                                    class="text-logo-teal focus:ring-logo-teal/20">
                                                <span class="text-xs font-semibold text-gray">Add all qualifying discounts</span>
                                            </label>
                                            <p class="text-xs text-gray/50 ml-5">All applicable group discounts are summed (capped at 100%).</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            {{-- ── Surcharge Settings ── --}}
                            <div class="pt-5 border-t border-lumot/20">
                                <h3 class="text-sm font-bold text-gray mb-3 flex items-center gap-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Monthly Surcharge Rate (%)</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Maximum Surcharge Rate (%)</label>
                                        <input type="number" name="max_surcharge_rate"
                                            value="{{ $settings['max_surcharge_rate']->value ?? 72 }}" step="1"
                                            min="0" max="100"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2
                                                   focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        <p class="text-xs text-gray/50 mt-1">Maximum total surcharge allowed (Default: 72%)</p>
                                    </div>
                                </div>
                            </div>
                ...
                <div id="receipt" class="mb-6">
                    <form action="{{ route('bpls.settings.update-receipt') }}" method="POST">
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                <p class="text-xs text-gray/50 mb-4">These values appear at the top of every printed receipt.</p>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Office Name <span class="text-red-400">*</span></label>
                                        <input type="text" name="receipt_office_name"
                                            value="{{ $settings['receipt_office_name']->value ?? 'Office of the Treasurer' }}"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Header Line 3 (Province / Location)</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Agency Name / Code <span class="text-red-400">*</span></label>
                                        <input type="text" name="receipt_agency_name"
                                            value="{{ $settings['receipt_agency_name']->value ?? 'MTO-Majayjay' }}"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Accountable Form Label</label>
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                <div class="mb-4">
                                    <label class="block text-xs font-bold text-gray mb-1">Received Text <span class="text-red-400">*</span></label>
                                    <input type="text" name="receipt_received_text"
                                        value="{{ $settings['receipt_received_text']->value ?? 'Received the amount stated above' }}"
                                        class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray mb-1">Footer Note <span class="text-gray/40 font-normal">(optional)</span></label>
                                    <textarea name="receipt_footer_note" rows="2"
                                        placeholder="e.g. This is a system-generated receipt. No signature required."
                                        class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">{{ $settings['receipt_footer_note']->value ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                <p class="text-xs text-gray/50 mb-5">Signatory 1 is always shown. Leave the name blank to auto-use the logged-in cashier's name.</p>

                                <div class="mb-4 p-4 bg-logo-teal/5 border border-logo-teal/20 rounded-xl">
                                    <p class="text-xs font-extrabold uppercase tracking-wide text-logo-teal mb-3">Signatory 1 — Always Shown</p>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                        ...
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Title / Position <span class="text-red-400">*</span></label>
                                ...
                                <div class="mb-4 p-4 border border-lumot/20 rounded-xl" x-data="{ enabled: {{ ($settings['receipt_signatory2_enabled']->value ?? '0') === '1' ? 'true' : 'false' }} }">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-extrabold uppercase tracking-wide text-gray">Signatory 2</p>
                                        <label class="flex items-center gap-2 cursor-pointer select-none">
                                            <span class="text-xs text-gray/60" x-text="enabled ? 'Enabled' : 'Disabled'"></span>
                                            <input type="checkbox" name="receipt_signatory2_enabled" value="1" x-model="enabled"
                                            ...
                                            <div class="relative w-9 h-5 rounded-full cursor-pointer transition-colors"
                                                :class="enabled ? 'bg-logo-teal' : 'bg-gray/20'" @click="enabled = !enabled">
                                                <div class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform"
                                                    :class="enabled ? 'translate-x-4' : 'translate-x-0'"></div>
                                            </div>
                                        </label>
                                    </div>
                                    <div x-show="enabled" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Name</label>
                                            <input type="text" name="receipt_signatory2_name" value="{{ $settings['receipt_signatory2_name']->value ?? '' }}"
                                                class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Title / Position</label>
                                            <input type="text" name="receipt_signatory2_title" value="{{ $settings['receipt_signatory2_title']->value ?? '' }}"
                                                class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 border border-lumot/20 rounded-xl" x-data="{ enabled: {{ ($settings['receipt_signatory3_enabled']->value ?? '0') === '1' ? 'true' : 'false' }} }">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-extrabold uppercase tracking-wide text-gray">Signatory 3</p>
                                        <label class="flex items-center gap-2 cursor-pointer select-none">
                                            <span class="text-xs text-gray/60" x-text="enabled ? 'Enabled' : 'Disabled'"></span>
                                            <input type="checkbox" name="receipt_signatory3_enabled" value="1" x-model="enabled"
                                    ...
                                    <div x-show="enabled" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Name</label>
                                            <input type="text" name="receipt_signatory3_name" value="{{ $settings['receipt_signatory3_name']->value ?? '' }}"
                                                class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Title / Position</label>
                                            <input type="text" name="receipt_signatory3_title" value="{{ $settings['receipt_signatory3_title']->value ?? '' }}"
                                                class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            <div class="bg-green text-white text-center py-2.5">
                                <p class="text-xs font-extrabold tracking-wide uppercase">Account &amp; Fund Code Defaults</p>
                            </div>
                            <div class="p-5">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Surcharge Account Code</label>
                                        <input type="text" name="receipt_surcharge_code"
                                            value="{{ $settings['receipt_surcharge_code']->value ?? '631-008' }}"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Backtax Account Code</label>
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Receipt Width (px)</label>
                                        <input type="number" name="receipt_width_px"
                                            value="{{ $settings['receipt_width_px']->value ?? '360' }}" min="280" max="600"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Minimum Fee Rows (filler lines)</label>
                                        <input type="number" name="receipt_min_fee_rows"
                                            value="{{ $settings['receipt_min_fee_rows']->value ?? '8' }}" min="1" max="20"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                    </div>
                                </div>
                                <div class="space-y-3">
                                    <label class="flex items-start gap-3 cursor-pointer">
                                        ...
                                        <div>
                                            <span class="text-sm font-semibold text-gray">Show Advance Discount Badge</span>
                                            <p class="text-xs text-gray/50">Display a green banner when an advance discount is applied</p>
                                        </div>
                                    </label>
                                    <label class="flex items-start gap-3 cursor-pointer">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray/50">Print total spelled out (e.g. "One Thousand Five Hundred Pesos Only")</p>
                                        </div>
                                    </label>
                                    <label class="flex items-start gap-3 cursor-pointer">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray/50">Print the remarks / notes row on the receipt</p>
                                        </div>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="mb-4 flex items-start gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                            ...
                            <p class="text-xs text-yellow-800">After saving, open any payment receipt to see your changes reflected immediately.</p>
                        </div>
