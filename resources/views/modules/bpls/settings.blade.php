            ...
            <div class="min-h-screen bg-gradient-to-br from-bluebody via-white to-blue/5 p-4">
                        ...
                        <form action="{{ route('bpls.settings.update-general') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="current_tax_year"
                                    class="w-full md:w-64 text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                            ...
                            <div class="flex justify-end">
                                <button type="submit"
                                    class="flex items-center gap-2 px-6 py-2.5 bg-logo-teal text-white text-sm font-bold rounded-xl hover:bg-green transition-colors shadow-md shadow-logo-teal/20">
                    ...
                    <div class="p-5">
                        <form action="{{ route('bpls.settings.update-discount') }}" method="POST">
                            ...
                            <div class="mt-4 p-3 bg-blue-50 rounded-lg">
                                ...
                                @endphp
                                <p class="text-xs text-gray-600">Example: Quarterly payment of
                                    ₱{{ number_format($sampleAmount, 2) }} paid
                                    {{ $daysBefore }} days early =
                                    <span
                                        class="font-bold text-logo-green">₱{{ number_format($discountedAmount, 2) }}</span>
                                    ({{ $quarterlyRate }}% off)
                                </p>
                            </div>
                ...
                <div id="fees"
                    ...
                    <div class="p-5">
                        ...
                        <p class="text-sm text-gray/70 text-center py-8">Click the button above to manage fee rules,
                            rates, and computations.</p>
                    </div>
                </div>
                        ...
                        <form action="{{ route('bpls.settings.update-permit') }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label class="block text-xs font-bold text-gray mb-1">
                                    Permit Number Format <span class="text-red-400">*</span>
                                </label>
                                ...
                                <p class="text-xs text-gray/50 mt-1">
                                    Available tags: <code class="bg-gray/10 px-1 rounded">{year}</code>
                                    <code class="bg-gray/10 px-1 rounded">{id}</code>
                                    <code class="bg-gray/10 px-1 rounded">{muni}</code>
                                    <code class="bg-gray/10 px-1 rounded">{barangay_code}</code>
                                    — e.g. <strong>BPLS-{year}-{id}</strong> → BPLS-2026-000044
                                </p>
                            </div>

                            {{-- ── Business ID Format ── --}}
                            <div class="mb-4 p-4 bg-logo-teal/5 border border-logo-teal/20 rounded-xl">
                                <label class="block text-xs font-bold text-logo-teal mb-1">
                                    Business ID Format <span class="text-red-400">*</span>
                                </label>
                                <input type="text" name="business_id_format"
                                    value="{{ $settings['business_id_format']->value ?? 'BPLS-{year}-{id}' }}"
                                    class="w-full md:w-96 text-sm border border-lumot/30 rounded-xl px-3 py-2
                                           focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                <p class="text-xs text-gray/50 mt-1">
                                    This is the <strong>Business ID</strong> saved in the database and shown on the
                                    portal.
                                    Available tags: <code class="bg-gray/10 px-1 rounded">{year}</code>
                                    <code class="bg-gray/10 px-1 rounded">{id}</code>
                                    <code class="bg-gray/10 px-1 rounded">{muni}</code>
                                ...
                                {{-- Live preview --}}
                                @php
                                    $previewFormat = $settings['business_id_format']->value ?? 'BPLS-{year}-{id}';
                                    $previewResult = str_ireplace(
                                        ['{year}', '{id}', '{muni}', '{barangay_code}'],
                                        [now()->year, '000044', 'BPLS', 'BRG'],
                                        $previewFormat,
                                    );
                                @endphp
                                <p class="text-xs font-semibold text-logo-teal mt-2">
                                    Preview: <span class="font-extrabold">{{ $previewResult }}</span>
                                </p>
                            </div>
                        ...
                        <div x-show="addOpen" x-transition x-cloak
                            ...
                            <form action="{{ route('bpls.permit-signatories.store') }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="sort_order" value="0" min="0"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        <p class="text-[10px] text-gray/40 mt-1">Lower numbers appear first</p>
                                    </div>
                                </div>
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button type="submit"
                                        class="px-4 py-2 text-xs font-bold bg-logo-teal text-white rounded-xl hover:bg-green transition-colors">Save
                                        Signatory</button>
                                </div>
                            </form>
                        </div>
                    ...
                    <form action="{{ route('bpls.settings.update-receipt') }}" method="POST">
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Office Name <span
                                                class="text-red-400">*</span></label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Agency Name / Code <span
                                                class="text-red-400">*</span></label>
                        ...
                        <div class="bg-white rounded-2xl border border-lumot/20 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="p-5">
                                <div class="mb-4">
                                    <label class="block text-xs font-bold text-gray mb-1">Received Text <span
                                            class="text-red-400">*</span></label>
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-gray mb-1">Footer Note <span
                            ...
                            <div class="p-5">
                                ...
                                <div class="mb-4 p-4 bg-logo-teal/5 border border-logo-teal/20 rounded-xl">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label class="block text-xs font-bold text-gray mb-1">Title / Position
                                                <span class="text-red-400">*</span></label>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <label class="flex items-start gap-3 cursor-pointer">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray/50">Print total spelled out</p>
                                        </div>
                                    </label>
                                    <label class="flex items-start gap-3 cursor-pointer">
                                    ...
                                    <div class="flex justify-end">
                                        <button type="submit"
                                            class="flex items-center gap-2 px-6 py-2.5 bg-logo-teal text-white text-sm font-bold mb-10 rounded-xl hover:bg-green transition-colors shadow-md shadow-logo-teal/20">
                        ...
                        <div id="beneficiary-discounts"
                            ...
                            <div class="bg-green text-white text-center py-2.5">
                                <p class="text-xs font-extrabold tracking-wide uppercase">Beneficiary Discount Settings
                                </p>
                            </div>
                            <div class="p-5 items-center justify-center flex w-full">
                                <a href="benefits"
                                    class="px-2 py-2 rounded-xl bg-green text-white font-semibold">Click this to Edit
                                    Benefits Discount</a>
                            </div>
                        </div>
