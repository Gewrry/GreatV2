    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="min-h-screen bg-gradient-to-br from-bluebody via-white to-blue/5 p-4">
                ...
                <div id="beneficiary-discounts"
                    class="bg-white rounded-2xl border border-purple-200 shadow-sm overflow-hidden mb-6">
                    <div class="bg-purple-600 text-white text-center py-2.5">
                        <p class="text-xs font-extrabold tracking-wide uppercase">Beneficiary Discount Settings</p>
                    </div>
                    <div class="p-5">
                        <form action="{{ route('bpls.settings.update-beneficiary-discount') }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-5">
                                <label class="flex items-center gap-2">
                                    <input type="checkbox" name="beneficiary_discount_enabled" value="1"
                                        {{ ($settings['beneficiary_discount_enabled']->value ?? '0') == '1' ? 'checked' : '' }}
                                        class="rounded border-lumot/30 text-purple-600 focus:ring-purple-400/20">
                                    <span class="text-sm font-semibold text-gray">Enable Beneficiary Discounts</span>
                                </label>
                                <p class="text-xs text-gray/50 mt-1 ml-6">
                                    When enabled, qualifying owners (PWD, Senior, Solo Parent, 4Ps, vaccinated) receive
                                    a discount automatically applied on the Payment page.
                                </p>
                            </div>

                            {{-- Stack rule --}}
                            <div class="mb-5">
                                <label class="block text-xs font-bold text-gray mb-2">When Multiple Qualifications
                                    Apply</label>
                                <div class="flex gap-6">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="beneficiary_discount_stack" value="highest_only"
                                            ...
                                            class="text-purple-600 focus:ring-purple-400/20">
                                        <span class="text-sm text-gray">Use highest discount only</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="beneficiary_discount_stack" value="additive"
                                            {{ ($settings['beneficiary_discount_stack']->value ?? 'highest_only') === 'additive' ? 'checked' : '' }}
                                            class="text-purple-600 focus:ring-purple-400/20">
                                        <span class="text-sm text-gray">Stack all discounts (additive)</span>
                                    </label>
                                </div>
                                <p class="text-xs text-gray/50 mt-1">
                                    Example: Owner is both PWD (20%) and Senior (20%). Highest-only = 20%. Additive =
                                    40%.
                                </p>
                            </div>

                            {{-- Per-group rate table --}}
                            <div class="border border-lumot/20 rounded-xl overflow-hidden mb-5">
                                <div class="grid grid-cols-4 bg-lumot/20 px-4 py-2 border-b border-lumot/20">
                                    <p class="text-[10px] font-extrabold text-gray/70 uppercase col-span-1">Group</p>
                                    <p class="text-[10px] font-extrabold text-gray/70 uppercase text-center">Rate (%)
                                    </p>
                                    <p
                                        class="text-[10px] font-extrabold text-gray/70 uppercase text-center col-span-2">
                                        Apply Discount To</p>
                                </div>
                                ...
                                @php
                                    $beGroups = [
                                        ...
                                        [
                                            ...
                                            'emoji' => '🏠',
                                            'label' => '4Ps',
                                            'sub' => 'Pantawid Pamilyang Pilipino',
                                            'default_rate' => 10,
                                        ],
                                        [
                                            'key' => 'discount_10',
                                ...
                                @foreach ($beGroups as $grp)
                                    <div
                                        class="grid grid-cols-4 items-center px-4 py-3 {{ !$loop->last ? 'border-b border-lumot/10' : '' }} hover:bg-purple-50/20">
                                        <div class="flex items-center gap-2 col-span-1">
                                            <span class="text-base">{{ $grp['emoji'] }}</span>
                                            <div>
                                                <p class="text-xs font-bold text-gray">{{ $grp['label'] }}</p>
                                                <p class="text-[10px] text-gray/50">{{ $grp['sub'] }}</p>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-center gap-1">
                                            <input type="number" name="{{ $grp['key'] }}_discount_rate"
                                                value="{{ $settings[$grp['key'] . '_discount_rate']->value ?? $grp['default_rate'] }}"
                                                step="0.1" min="0" max="100"
                                                class="w-20 text-sm text-center border border-lumot/30 rounded-lg px-2 py-1.5
                                                       focus:outline-none focus:ring-2 focus:ring-purple-400/40">
                                            <span class="text-xs text-gray/50">%</span>
                                        </div>
                                        <div class="flex gap-4 col-span-2 justify-center">
                                            <label class="flex items-center gap-1.5 cursor-pointer">
                                                <input type="radio" name="{{ $grp['key'] }}_discount_apply_to"
                                                    value="total"
                                                    {{ ($settings[$grp['key'] . '_discount_apply_to']->value ?? 'total') === 'total' ? 'checked' : '' }}
                                                    class="text-purple-600">
                                                <span class="text-xs text-gray">Total Due</span>
                                            </label>
                                            <label class="flex items-center gap-1.5 cursor-pointer">
                                                <input type="radio" name="{{ $grp['key'] }}_discount_apply_to"
                                                    value="permit_only"
                                                    {{ ($settings[$grp['key'] . '_discount_apply_to']->value ?? 'total') === 'permit_only' ? 'checked' : '' }}
                                                    class="text-purple-600">
                                                <span class="text-xs text-gray">Mayor's Permit Only</span>
                                            </label>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            {{-- Info hint --}}
                            <div
                                class="mb-5 flex items-start gap-2 p-3 bg-purple-50 border border-purple-200 rounded-xl">
                                <span class="text-base shrink-0">💡</span>
                                <div class="text-xs text-purple-800 space-y-1">
                                    <p><strong>Total Due</strong> — applies the % to the full
                                        quarterly/semi-annual/annual payment amount.</p>
                                    <p><strong>Mayor's Permit Only</strong> — applies the % only to the permit fee
                                        portion (split proportionally from total).</p>
                                    <p>These rates are system defaults. The actual owner classification (PWD, Senior,
                                        etc.) is toggled per business on the Payment page.</p>
                                </div>
                            </div>

                            <div class="flex justify-end">
                                <button type="submit"
                                    class="flex items-center gap-2 px-6 py-2.5 bg-purple-600 text-white text-sm font-bold
                                           rounded-xl hover:bg-purple-700 transition-colors shadow-md shadow-purple-400/20">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                                    </svg>
                                    Save Beneficiary Discount Settings
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div id="permit-signatories"
                    ...
                    <div class="p-5">
                        ...
                        <div x-show="addOpen" x-transition x-cloak
                            ...
                            <p class="text-xs font-extrabold text-logo-teal uppercase tracking-wide mb-3">New Signatory
                            </p>
                            <form action="{{ route('bpls.permit-signatories.store') }}" method="POST">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Name <span
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-gray mb-1">Position / Designation
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="sort_order" value="0" min="0"
                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                        <p class="text-[10px] text-gray/40 mt-1">Lower numbers appear first in dropdown
                                        </p>
                                    </div>
                                </div>
                        ...
                        @if ($signatories->isEmpty())
                            <div class="py-8 text-center">
                                <svg class="w-10 h-10 text-lumot/30 mx-auto mb-2" fill="none" viewBox="0 0 24 24"
                            ...
                            <div class="space-y-2">
                                @foreach ($signatories as $sig)
                                    <div class="border border-lumot/20 rounded-xl overflow-hidden"
                                        ...
                                        {{-- Row --}}
                                        <div
                                            class="flex items-center justify-between px-4 py-3 {{ $sig->is_active ? 'bg-white' : 'bg-lumot/5' }}">
                                            <div class="flex items-center gap-3 min-w-0">
                                                <span
                                                    class="w-2 h-2 rounded-full shrink-0 {{ $sig->is_active ? 'bg-logo-green' : 'bg-lumot/40' }}"></span>
                                                <div class="min-w-0">
                                                    <p class="text-sm font-bold text-green truncate">
                                                        {{ $sig->name }}</p>
                                                    <p class="text-[11px] text-gray/50 truncate">
                                            ...
                                            <div class="flex items-center gap-2 shrink-0 ml-3">
                                                <span
                                                    class="text-[10px] font-bold px-2 py-0.5 rounded-full border {{ $sig->is_active ? 'bg-logo-green/10 text-logo-green border-logo-green/30' : 'bg-lumot/20 text-gray/50 border-lumot/30' }}">
                                                ...
                                                <button type="button" @click="editOpen = !editOpen"
                                                    class="p-1.5 rounded-lg bg-logo-teal/10 text-logo-teal hover:bg-logo-teal/20 transition-colors"
                                                    title="Edit">
                                                ...
                                                <form
                                                    action="{{ route('bpls.permit-signatories.destroy', $sig->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Delete {{ addslashes($sig->name) }}?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        class="p-1.5 rounded-lg bg-red-50 text-red-400 hover:bg-red-100 hover:text-red-600 transition-colors"
                                                        title="Delete">
                                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                            stroke="currentColor" stroke-width="2">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <div x-show="editOpen" x-transition x-cloak
                                            class="border-t border-lumot/20 bg-lumot/5 p-4">
                                            <form action="{{ route('bpls.permit-signatories.update', $sig->id) }}"
                                                method="POST">
                                                @csrf @method('PUT')
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                                    <div>
                                                        <label class="block text-xs font-bold text-gray mb-1">Name
                                                            <span class="text-red-400">*</span></label>
                                                        <input type="text" name="name" required maxlength="150"
                                                            value="{{ $sig->name }}"
                                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                                    ...
                                                    <div>
                                                        <label class="block text-xs font-bold text-gray mb-1">Position
                                                            <span class="text-red-400">*</span></label>
                                                        <input type="text" name="position" required
                                                            maxlength="150" value="{{ $sig->position }}"
                                                            class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                                    </div>
                                                    <div>
                                                        <label
                                                            class="block text-xs font-bold text-gray mb-1">Department</label>
                                                        <input type="text" name="department" maxlength="150"
                                                    ...
                                                    <div class="grid grid-cols-2 gap-3">
                                                        <div>
                                                            <label class="block text-xs font-bold text-gray mb-1">Sort
                                                                Order</label>
                                                            <input type="number" name="sort_order" min="0"
                                                                value="{{ $sig->sort_order }}"
                                                                class="w-full text-sm border border-lumot/30 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-logo-teal/40">
                                                        </div>
